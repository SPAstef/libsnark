...
namespace libsnark
{
    ...
    knapsack_CRH_with_field_out_gadget<FieldT>::knapsack_CRH_with_field_out_gadget(
        protoboard<FieldT> &pb, const size_t input_len, const block_variable<FieldT> &input_block,
        const pb_linear_combination_array<FieldT> &output, const std::string &annotation_prefix) :
        gadget<FieldT>(pb, annotation_prefix), input_len(input_len),
        dimension(knapsack_dimension<FieldT>::dimension), input_block(input_block), output(output)
    {
    ...
    template<typename FieldT>
    std::vector<FieldT>
    knapsack_CRH_with_field_out_gadget<FieldT>::get_hash(const libff::bit_vector &input)
    {
    ...
    knapsack_CRH_with_bit_out_gadget<FieldT>::knapsack_CRH_with_bit_out_gadget(
        protoboard<FieldT> &pb, const size_t input_len, const block_variable<FieldT> &input_block,
        const digest_variable<FieldT> &output_digest, const std::string &annotation_prefix) :
        gadget<FieldT>(pb, annotation_prefix), input_len(input_len),
        dimension(knapsack_dimension<FieldT>::dimension), input_block(input_block),
        output_digest(output_digest)
    {
        ...
        {
            output[i].assign(
                pb, pb_packing_sum<FieldT>(pb_variable_array<FieldT>(
                        output_digest.bits.begin() + i * FieldT::ceil_size_in_bits(),
    ...
    template<typename FieldT>
    void
    knapsack_CRH_with_bit_out_gadget<FieldT>::generate_r1cs_constraints(const bool enforce_bitness)
    {
    ...
    template<typename FieldT>
    libff::bit_vector
    knapsack_CRH_with_bit_out_gadget<FieldT>::get_hash(const libff::bit_vector &input)
    {
        const std::vector<FieldT> hash_elems = knapsack_CRH_with_field_out_gadget<FieldT>::get_hash(
            input);
    ...
    template<typename FieldT>
    size_t
    knapsack_CRH_with_bit_out_gadget<FieldT>::expected_constraints(const bool enforce_bitness)
    {
    ...
    template<typename FieldT>
    void test_knapsack_CRH_with_bit_out_gadget_internal(const size_t dimension,
                                                        ...
                                                        const libff::bit_vector &digest_bits)
    {
        ...
        digest_variable<FieldT> output_digest(
            pb, knapsack_CRH_with_bit_out_gadget<FieldT>::get_digest_len(), "output_digest");
        knapsack_CRH_with_bit_out_gadget<FieldT> H(pb, input_bits.size(), input_block,
                                                   output_digest, "H");
...
#else  // NDEBUG
        printf("All tests here depend on assert() which is disabled by -DNDEBUG. Please recompile "
               "and run again.\n");

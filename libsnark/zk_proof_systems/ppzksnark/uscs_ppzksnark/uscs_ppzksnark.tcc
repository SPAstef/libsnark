...
namespace libsnark
{
    ...
    bool
    uscs_ppzksnark_proving_key<ppT>::operator==(const uscs_ppzksnark_proving_key<ppT> &other) const
    {
    ...
    uscs_ppzksnark_generator(const uscs_ppzksnark_constraint_system<ppT> &cs)
    {
        ...
        ssp_instance_evaluation<libff::Fr<ppT>> ssp_inst = uscs_to_ssp_instance_map_with_evaluation(
            cs, t);
        ...
        const size_t g1_exp_count = Vt_table.size() + Vt_table_minus_Xt_table.size() +
                                    Ht_table.size();
        ...
        libff::G1_vector<ppT> V_g1_query = batch_exp(libff::Fr<ppT>::size_in_bits(), g1_window,
        ...
        libff::G1_vector<ppT> alpha_V_g1_query = batch_exp_with_coeff(
            libff::Fr<ppT>::size_in_bits(), g1_window, g1_table, alpha, Vt_table_minus_Xt_table);
#ifdef USE_MIXED_ADDITION
        ...
        libff::G1_vector<ppT> H_g1_query = batch_exp(libff::Fr<ppT>::size_in_bits(), g1_window,
                                                     g1_table, Ht_table);
#ifdef USE_MIXED_ADDITION
        ...
        libff::G2_vector<ppT> V_g2_query = batch_exp(libff::Fr<ppT>::size_in_bits(), g2_window,
                                                     g2_table, Vt_table);
#ifdef USE_MIXED_ADDITION
        ...
        libff::G1_vector<ppT> encoded_IC_values = batch_exp(
            libff::Fr<ppT>::size_in_bits(), g1_window, g1_table,
        ...
        uscs_ppzksnark_verification_key<ppT> vk = uscs_ppzksnark_verification_key<ppT>(
        ...
        uscs_ppzksnark_proving_key<ppT> pk = uscs_ppzksnark_proving_key<ppT>(
            std::move(V_g1_query), std::move(alpha_V_g1_query), std::move(H_g1_query),
            std::move(V_g2_query), std::move(cs_copy));
                          ...
                          const uscs_ppzksnark_auxiliary_input<ppT> &auxiliary_input)
    {
        ...
        libff::G2<ppT> V_g2 = pk.V_g2_query[0] +
                              ssp_wit.d * pk.V_g2_query[pk.V_g2_query.size() - 1];
        ...
        V_g1 = V_g1 + libff::multi_exp_with_mixed_addition<libff::G1<ppT>, libff::Fr<ppT>,
        ...
        const uscs_ppzksnark_proof<ppT> &proof)
    {
        ...
        if (!proof.is_well_formed())
        {
            ...
            {
                libff::print_indent();
                printf("At least one of the proof components is not well-formed.\n");
            }
            result = false;
        }
        ...
        libff::GT<ppT> SSP = ppT::final_exponentiation(SSP_1.unitary_inverse() * SSP_2 *
        ...
        libff::Fqk<ppT> alpha_V_1 = ppT::miller_loop(proof_V_g1_precomp,
                                                     pvk.vk_alpha_tilde_g2_precomp);
        libff::Fqk<ppT> alpha_V_2 = ppT::miller_loop(proof_alpha_V_g1_precomp,
                                                     pvk.vk_tilde_g2_precomp);

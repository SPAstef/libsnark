...
namespace libsnark
{
        ...
        for (size_t c = 0; c < constraints.size(); ++c)
        {
            ...
            if (!(ares * bres == cres))
            {
                ...
                printf("constraint %zu (%s) unsatisfied\n", c,
                       (it == constraint_annotations.end() ? "no annotation" : it->second.c_str()));
                printf("<a,(1,x)> = ");
                ares.print();
                printf("<b,(1,x)> = ");
                bres.print();
    ...
    void r1cs_constraint_system<FieldT>::swap_AB_if_beneficial()
    {
        ...
        else if (!libff::inhibit_profiling_info)
        {
            libff::print_indent();
    ...
    template<typename FieldT>
    bool
    r1cs_constraint_system<FieldT>::operator==(const r1cs_constraint_system<FieldT> &other) const
    {
